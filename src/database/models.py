...
class Company(Base, TenantMixin, SoftDeleteMixin, AuditableMixin):
    """
    Represents a B2B client organization (e.g., 'Ingenio del Cauca').
    """
    __tablename__ = "companies"

    id = Column(BigInteger, primary_key=True, autoincrement=True)
    name = Column(String(255), nullable=False, index=True)
    fiscal_id = Column(String(50), nullable=True) # E.g. NIT, RUT, VAT
    crm_company_id = Column(String(255), nullable=True, index=True) # ID from external CRM
    
    
    # Relaciones
    divisions = relationship("CompanyDivision", back_populates="company", cascade="all, delete-orphan")

class CompanyDivision(Base, TenantMixin, SoftDeleteMixin, AuditableMixin):
    """
    Represents a specific department, site, or laboratory within a Company (e.g., 'Laboratorio de Aguas').
    """
    __tablename__ = "company_divisions"

    id = Column(BigInteger, primary_key=True, autoincrement=True)
    company_id = Column(BigInteger, ForeignKey("companies.id"), nullable=False, index=True)
    name = Column(String(255), nullable=False)
    crm_division_id = Column(String(255), nullable=True, index=True) # ID from external CRM
...
class User(Base, TenantMixin, SoftDeleteMixin, AuditableMixin):
    ...
    # LDAP-like Organizational Structure (B2B Hierarchy: Company -> Division -> User)
    division_id = Column(BigInteger, ForeignKey("company_divisions.id"), nullable=True, index=True)
    crm_contact_id = Column(String(255), nullable=True, index=True) # ID from external CRM
    ...
    # Relationships
    division = relationship("CompanyDivision", back_populates="users")
...
class Message(Base, TenantMixin, AuditableMixin):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    conversation_id = Column(BigInteger, ForeignKey("conversations.id"), index=True, nullable=False)
...
class OpportunityProductRecommendation(Base, TenantMixin, AuditableMixin):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    opportunity_id = Column(BigInteger, ForeignKey("leads_opportunities.id"), index=True, nullable=False)
